...
render screen camera acc = zipWith
...
traceRay limit scene primaryRay =
  let T3 (T2 _ seed) result _ =
          iterate limit check (T3 primaryRay initialColor initialThroughput)
  in  T2 result seed
 where
  ...
  calculate
    ...
    -> Exp (Normal, Material)
    -> Exp ((RayF, Word32), Color, V3 Float)
  calculate (T3 (T2 ray seed) result throughput) (T2 (Ray_ intersection iNormal) iMaterial)
    = let
        ...
        T2 nextDirection brdf'     = caseof
          ...
          , ( isGlossy
            ...
            , let
                ...
                next = rotate
                  (anglesToQuaternion $ (1 - p) *^ rotationVector)
                  reflection
...
closestIntersection scene ray =
  fmap snd $ expMinWith (maybe infinite fst) $ mapScene
